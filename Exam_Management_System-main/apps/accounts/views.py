...
def student_register_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Read department from the registration form (optional)
            dept = form.cleaned_data.get('department') or 'Undeclared'
...
def admin_register_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.username = _generate_username_from_email(form.cleaned_data.get('email'))
            user.role = 'admin'
            # Store chosen department (optional)
            user.department = form.cleaned_data.get('department') or None
            user.save()
...
def login_view(request):
    ...
    POST: Process login
    Query param 'role' can be 'student' or 'admin' to set the login mode.
    """
    if request.user.is_authenticated:
        return redirect('dashboard')
    
    # Get role from query params (default: 'student')
    login_role = request.GET.get('role', 'student').lower()
    if login_role not in ['student', 'admin']:
        login_role = 'student'
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                # Check if user role matches the login mode
                if login_role == 'student' and not user.is_student_user:
                    messages.error(request, 'This account is not a student account. Please use the Admin login.')
                elif login_role == 'admin' and not user.is_admin_user:
                    messages.error(request, 'This account is not an admin account. Please use the Student login.')
                else:
                    login(request, user)
    ...
    return render(request, 'accounts/login.html', {'form': form, 'login_role': login_role})
...
def profile_view(request):
    ...
    if request.user.is_student_user:
        ...
        context = {
            ...
            'completed_exams': len(past_exams),
            'upcoming_exams': upcoming_exams,
            'past_exams': past_exams,
            'average_score': '—',  # Can be calculated from results if needed
        }
...
def profile_update_view(request):
    """Update user profile information. Works for both students and admins."""
    
    # For students: update StudentProfile
    if request.user.is_student_user:
    ...
    # For admins: update basic user info and profile picture
        ...
        if request.method == 'POST':
            ...
            logger.info('Admin profile updated for user %s', user.pk)
            messages.success(request, 'Profile updated successfully!')
            return redirect('profile')
